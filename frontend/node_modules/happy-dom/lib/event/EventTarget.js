...
export default class EventTarget {
    ...
    #callDispatchEventListeners(event) {
        ...
        if (event.eventPhase !== EventPhaseEnum.capturing) {
            ...
            const propertyEventListeners = this[PropertySymbol.propertyEventListeners];
            ...
            const hasClassDefinedHandler = !propertyEventListeners &&
                Object.hasOwn(this, onEventName) &&
                this.constructor.name !== 'EventTarget';
            if (propertyEventListeners || hasClassDefinedHandler) {
                const eventListener = propertyEventListeners?.get(onEventName) ?? this[onEventName];
