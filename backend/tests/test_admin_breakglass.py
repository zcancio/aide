...
class TestAdminBreakglass:
    ...
    async def test_admin_can_access_any_aide(self, initialize_pool, async_client):
        """Admins should be able to access any aide via breakglass."""
        # Create an admin user
        admin = await user_repo.create(email=f"admin-{uuid4()}@example.com")
        # Manually set admin flag (in production, this would be done via database)
        ...
        admin = await user_repo.get(admin.id)
    ...
    async def test_breakglass_access_is_audited(self, initialize_pool, async_client):
        ...
        audit_logs = await admin_audit_repo.list_audit_logs(admin_user_id=admin.id, limit=10)
        ...
        assert log_entry.admin_user_id == admin.id
    ...
    async def test_breakglass_requires_valid_reason(self, initialize_pool, async_client):
    ...
    async def test_breakglass_rejects_mismatched_aide_id(self, initialize_pool, async_client):
    ...
    async def test_breakglass_returns_404_for_nonexistent_aide(self, initialize_pool, async_client):
    ...
    async def test_audit_log_count_endpoint(self, initialize_pool, async_client):
