...
function humanizeText(key){return key.replace(/_/g," ").replace(/\b\w/g,c=>c.toUpperCase())}function fmtValueText(val,type){if(val===null||val===undefined)return"—";if(typeof val==="boolean")return val?"Yes":"No";if(Array.isArray(val))return val.join(", ");return String(val)}function renderEntityText(entity,schema){const fields=schema||{};const checkField=Object.keys(fields).find(f=>["done","checked","complete"].includes(f)&&h(fields[f])==="bool");if(checkField){const checked=entity[checkField];const label=entity.name||entity.title||entity.id||"?";return`${checked?"✓":"○"} ${label}`}const nameField=entity.name||entity.title||null;const otherFields=Object.entries(entity).filter(([k])=>!k.startsWith("_")&&k!=="name"&&k!=="title").filter(([k])=>k in fields).map(([k,v])=>`${humanizeText(k)}: ${fmtValueText(v,fields[k])}`);if(nameField&&otherFields.length>0){return`  ${nameField}  │  ${otherFields.join("  │  ")}`}else if(nameField){return`  ${nameField}`}else{return`  ${otherFields.join("  │  ")}`}}function renderText(snapshot){const lines=[];const title=snapshot.meta?.title||"Untitled";lines.push(title);lines.push("═".repeat(Math.min(title.length,60)));lines.push("");for(const[colId,col]of Object.entries(snapshot.collections||{})){if(col._removed)continue;const entities=Object.entries(col.entities||{}).filter(([,e])=>!e._removed).sort((a,b)=>(a[1]._created_seq||0)-(b[1]._created_seq||0));if(entities.length===0)continue;lines.push(col.name||colId);lines.push("─".repeat(Math.min((col.name||colId).length,40)));for(const[eid,entity]of entities){lines.push(renderEntityText(entity,col.schema))}lines.push("")}const pinned=(snapshot.annotations||[]).filter(a=>a.pinned);if(pinned.length>0){lines.push("Notes");lines.push("─────");for(const ann of pinned){lines.push(`• ${ann.note}`)}lines.push("")}return lines.join("\n").trimEnd()}module.exports.renderText=renderText;
