...
function mkTree(ents) {
  if (!ents) return [];
  const all = Object.values(ents).filter(e => e && e.id);
  const ch = {};
  all.forEach(e => { const p = e.parent || "root"; (ch[p] = ch[p] || []).push(e); });
  const out = [];
  const walk = (pid, d) => { (ch[pid] || []).forEach(c => { out.push({ ...c, depth: d }); walk(c.id, d + 1); }); };
  walk("root", 0);
  return out;
}
...
function eDiff(b, a) {
  ...
  // Detect prop changes on entities that exist in both
  const mod = [];
  const modDetails = {};
  for (const k of ak) {
...
export default function Viewer() {
  ...
  const tree = useMemo(() => mkTree(t.snapshot_after?.entities), [t.snapshot_after]);
          ...
          {tab === "snapshot" && <>
            ...
            <div style={{ marginTop:14 }}>
              ...
              <div>
                ...
                {diff.mod.map(k=><span key={k} style={{ display:"inline-block", padding:"2px 7px", borderRadius:4, background:"#1e3a5f", border:"1px solid #2563eb", color:"#93c5fd", fontSize:11, fontFamily:mono, marginRight:5, marginBottom:4 }} title={(diff.modDetails[k]||[]).join(", ")}>~ {k}</span>)}
          ...
          {tab === "tree" && <>
            ...
            {tree.length > 0 ? (
              <div style={{ background:"#08080f", border:"1px solid #1e1e2e", borderRadius:6, padding:12 }}>
                {tree.map((e,i)=>{ const nw=diff.add.includes(e.id); const md=diff.mod.includes(e.id); const clr=nw?"#6ee7b7":md?"#93c5fd":"#cbd5e1"; return (
                  <div key={i} style={{ paddingLeft:e.depth*20, paddingTop:3, paddingBottom:3, fontSize:12, fontFamily:mono }}>
                    <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                      <span style={{ color:"#475569" }}>{e.depth>0?"├─":"◆"}</span>
                      <span style={{ color:clr, fontWeight:nw||md?700:400 }}>{e.id}</span>
                      ...
                      {e.display && <span style={{ fontSize:10, padding:"0 4px", marginLeft:2, borderRadius:3, background:"#1e293b", color:"#94a3b8" }}>{e.display}</span>}
                    </div>
                    {e.props && Object.keys(e.props).length>0 && <div style={{ paddingLeft:20, fontSize:11, color:"#475569", fontFamily:mono }}>
                      {Object.entries(e.props).slice(0,5).map(([k,v])=>{
                        const changes = diff.modDetails[e.id] || [];
                        const isChanged = changes.some(c => c.startsWith(k+":") || c === "+"+k);
                        return <span key={k} style={{ marginRight:8, color:isChanged?"#93c5fd":"#475569", fontWeight:isChanged?600:400 }}>{k}={JSON.stringify(v).slice(0,24)}</span>;
                      })}
                    </div>}
                  </div>
                );})}
