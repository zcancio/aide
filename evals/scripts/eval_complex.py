#!/usr/bin/env python3
"""
Complex scenario eval runner.

Runs complex conversational scenarios where state builds across turns.
Each turn's output becomes the next turn's snapshot context.

Usage:
  # Run all complex scenarios
  python eval_complex.py

  # Run specific scenario
  python eval_complex.py --scenario christmas_week_realistic

  # Verbose: show full output per turn
  python eval_complex.py -v

  # Save run artifacts for review
  python eval_complex.py --save

  # Test candidate prompt version
  python eval_complex.py --prompt-version v2
...
def main():
    p = argparse.ArgumentParser(description="Complex scenario eval")
    ...
    save_dir = Path(args.output_dir) / f"complex_{ts}" if args.save else None
    if save_dir:
    ...
    print("AIde Complex Scenario Eval")
    ...
    # Final summary
    print(f"\n\n{'=' * 70}")
    print("COMPLEX SCENARIO SUMMARY")
    print(f"{'=' * 70}")
