...
mid-build. These scenarios test the full conversational loop:
  ...
  Turn 1: L4 creates initial structure (no schema exists yet)
...
Each turn specifies:
  ...
  - expected_tier: which tier should handle this (L3 for mutations, L4 for queries/first turn)
...
GRADUATION_REALISTIC = {
    ...
    "turns": [
        ...
        # ── Turn 2: Adds basic details ──
        {
            ...
            "notes": "L3 should find the details entity and update it. Should NOT create a new entity. "
        ...
        # ── Turn 3: Guest info starts flowing ──
        {
            ...
            "notes": "This is the interesting one. If Turn 1 didn't create a guest section, this needs "
            "new structure. Either way: 3 entities with names, linda/bob = confirmed, "
            "james = maybe/pending. Accept L3 or L4 depending on Turn 1's output.",
            "accept_tiers": ["L3", "L4"],
        },
        # ── Turn 4: More guests, casual style ──
        {
            ...
            "notes": "L3 adds 3 more guests. 'prob' for dave means status should be pending/maybe. "
        ...
        # ── Turn 7: Batch of food items ──
        {
            ...
            "notes": "L3 creates food entities. Carne asada assigned to Carlos, drinks to Maria. "
            "Dessert and sides should exist but be unassigned/TBD. Tests mixed assigned + unassigned.",
        },
        # ── Turn 8: Contradiction/correction ──
        {
            "message": "actually scratch that - maria's doing a fruit platter, not drinks. bob said he'll handle drinks",  # noqa: E501
            "expected_tier": "L3",
            "checks": {
                "updates_maria": True,  # maria's food item changes
                "updates_bob_or_drinks": True,  # drinks reassigned to bob
            },
            "notes": "Tests correction handling. L3 should update Maria's assignment from drinks to "
            "fruit platter, and assign drinks to Bob. Should NOT delete and recreate — should update.",
        },
        # ── Turn 9: Todo list ──
        {
            "message": "things we still gotta do: book the park pavilion, order a cake, get decorations, figure out parking",  # noqa: E501
            "expected_tier": "L3",
            "checks": {
            ...
            "notes": "New section (todo/checklist) → L3. Should create "
        ...
        # ── Turn 10: Quick update ──
        {
            ...
            "notes": "Classic L3 — find the 'book pavilion' task, set done=true. "
...
POKER_REALISTIC = {
    ...
    "turns": [
        ...
        },
        {
            ...
            "Tests whether L3 handles the sub correctly.",
        },
    ],
}
...
GROCERY_REALISTIC = {
    ...
    "turns": [
        ...
        },
        {
            ...
            "notes": "'got' = done. Should check off milk and eggs. Classic L3.",
        },
        {
            "message": "what's left",
            "expected_tier": "L4",
        ...
        },
        {
            ...
            "expected_tier": "L3",
            "checks": {"creates_5_items": True, "expect_in_output": ["tonight"]},
        ...
        },
        {
            ...
            "expected_tier": "L3",
            "checks": {"updates_chicken": True, "expect_in_output": ["2", "bone"]},
...
RENOVATION_REALISTIC = {
    ...
    "turns": [
        ...
        },
        {
            ...
            "L3 handles the update, L4 if no schema exists yet.",
        },
        {
            ...
            "a table, not dumped flat under page. L3 handles this, L4 if first message"
        ...
        {
            "message": "plumber can start march 10, electrician march 3. need to get countertops measured before either",  # noqa: E501
            "expected_tier": "L3",
        ...
        },
        {
            ...
            "expected_tier": "L3",
            "checks": {"updates_existing": True, "expect_in_output": ["11000|11k"]},
        ...
        },
        {
            ...
            "expected_tier": "L3",
            "checks": {"uses_rel_for_selection": True, "expect_in_output": ["woodworks", "9500"]},
        ...
        },
        {
            ...
            "expected_tier": "L3",
            "checks": {"updates_dates": True, "expect_in_output": ["17"]},
        ...
        },
        {
            ...
            "expected_tier": "L3",
            "checks": {"creates_budget_item": True, "marks_prereq_done": True, "expect_in_output": ["3200"]},
            "accept_tiers": ["L3", "L4"],
...
FLU_TRACKER = {
    ...
    "turns": [
        ...
        {
            "message": "ringo sunday dec 29: 1030am 103.7 tylenol, 135pm 101.5 motrin, 520pm 101 tylenol, 820pm 102 motrin",  # noqa: E501
            "expected_tier": "L3",
            "checks": {"batch_creates": True, "min_entities": 4, "expect_in_output": ["103.7", "101.5", "101", "102"]},
            "accept_tiers": ["L3", "L4"],
        ...
        # ── Tuesday Dec 31: New day entries ──
        {
            ...
            "expected_tier": "L3",
            "checks": {"appends_not_updates": True, "expect_in_output": ["101", "99.7"]},
        ...
        },
        {
            ...
            "expected_tier": "L3",
            "checks": {"appends_not_updates": True, "expect_in_output": ["100.5"]},
        ...
        # ── Retroactive correction ──
        {
            ...
            "expected_tier": "L3",
            "checks": {"updates_existing": True, "expect_in_output": ["101.5"]},
...
# ---------------------------------------------------------------------------
# Christmas Week Planner — day-by-day family holiday coordination
# ---------------------------------------------------------------------------
